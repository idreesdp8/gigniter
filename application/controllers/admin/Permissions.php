	...
	class Permissions extends CI_Controller{
		...
		public function __construct(){
			...
			if(isset($vs_id) && (isset($vs_role_id) && $vs_role_id>=1)){
				/* ok */ 
				$res_nums = $this->general_model->check_controller_permission_access('Admin/Permissions',$vs_role_id,'1');
				if($res_nums>0){
					/* ok */ 
				}else{
					redirect('/');
			...
			if(isset($_POST) && !empty($_POST)){
				...
				if($this->form_validation->run() == FALSE){
					...
					if(isset($_POST['saves_and_new'])){
						redirect("admin/permissions/add");
					}else{
						redirect("admin/permissions/index");	
					}
		...
		function update($args1=''){ 
			...
			if(isset($_POST) && !empty($_POST)){
				...
				if($this->form_validation->run() == FALSE){
					// validation fail
					$this->load->view('admin/permissions/update',$data);
				}else if((isset($args1) && $args1!='') && (isset($rec_nums) && $rec_nums>0)){
					$this->session->set_flashdata('error_msg','Error: This moodule and role data are already added!');
					$this->load->view('admin/permissions/update',$data);
