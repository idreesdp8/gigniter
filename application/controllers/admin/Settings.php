		...
		if (isset($_POST) && !empty($_POST)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if($password && $password != '') {
					$data['password'] = $this->general_model->safe_ci_encoder($password);
				}
				$res = $this->users_model->update_user_data($vs_id, $data);
				if (isset($res)) {
					$cstm_sess_data = array(
						'us_fullname' => ($fname ? ucfirst($fname) : '') . ' ' . ($lname ? ucfirst($lname) : ''),
						'us_fname' => ($fname ? ucfirst($fname) : ''),
						'us_lname' => ($lname ? ucfirst($lname) : ''),
					);
