...
class Categories extends CI_Controller
{
	...
	function trash($id)
	{
		...
		$this->session->set_flashdata('deleted_msg', 'Category is deleted');
	...
	function add()
	{
		...
		// form validation
		$this->form_validation->set_rules("label", "Category Name", "trim|required|xss_clean");
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($record) {
				$this->session->set_flashdata('warning_msg', 'Warning: This category is already added!');
			} else {
				...
				if (isset($res)) {
					...
					$this->session->set_flashdata('error_msg', 'Error while adding Category!');
				}
			}
			redirect("admin/categories");
		}
