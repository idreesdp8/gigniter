...
class Gig_statuses extends CI_Controller
{
	...
	function index()
	{
		...
		$this->load->view('admin/gig_statuses/index', $data);
	...
	function trash($id)
	{
		...
		$this->session->set_flashdata('deleted_msg', 'Gig Status is deleted');
		redirect('admin/gig_statuses');
		// } else {
		// 	$this->load->view('admin/no_permission_access');
		// }
	}
	...
	function add()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($record) {
				$this->session->set_flashdata('warning_msg', 'Warning: This gig status is already added!');
			} else {
				...
				// die();
				if (isset($res)) {
					$this->session->set_flashdata('success_msg', 'Gig status added successfully!');
				} else {
					$this->session->set_flashdata('error_msg', 'Error while adding Gig status!');
				}
			}
			redirect("admin/gig_statuses");
		}
	...
	function update()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			// } else {
				...
				if (isset($res)) {
					$this->session->set_flashdata('success_msg', 'Gig Status updated successfully!');
				} else {
					$this->session->set_flashdata('error_msg', 'Error while updating Gig Status!');
				}
			// }
			redirect("admin/gig_statuses");
		}
