...
class Stripe extends CI_Controller
{
    ...
    {
        $this->load->view('frontend/stripePayment/demo4');
    }
    ...
    function transfer4()
    {
        ...
        if ($success_md == 1) {
            ...
            // echo json_encode($charge);
            // die();
            $portal = \Stripe\Invoice::create([
                'customer' => $customer->id,
                'collection_method' => 'send_invoice',
                'due_date' => strtotime('next Monday'),
                // 'amount_due' => $total_charged,
                // 'currency' => 'usd',
            ]);

            $portalJson = $portal->jsonSerialize();
            //check whether the portal is successful
            //order details
            $out = [
                'status' => 1,
                'message' => 'Follow the below link to complete the transaction',
                // 'url' => $portalJson['url']
            ];
